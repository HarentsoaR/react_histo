...
import Update from "./depUpdate";
import Suppr from "./depSuppr";
import Add from "./depAdd";
import Recharts from './ItemChart';
...
function Index() {
  ...
  const historiqueEmploye = async (event) => {
    ...
    try {
      const response = await axios.get(`http://localhost:8088/affiche_dep?matricule=${matricule}`);
    console.log(response.data.data);
      ...
      if (response.data.data === null) {
        ...
        setErrorMat("Pas d'historique des départements pour ce matricule");
      } else {
        setErrorMat(response.data.data.length + " changement de département");
      }
  ...
  const handleItemAdded = (newDepartment) => {
        ...
        <div className="container-fluid">
          <div className="row">
            <div className="col-sm-3 col-md-6 col-lg-4">
              ...
              <p className="more">
  <strong>
    {data && data.length > 0 ? data[data.length - 1]?.id_pers.toString().padStart(5, '0') : ""}
  </strong>
</p>
<p>
  Prénom: <strong>{data && data.length > 0 ? data[data.length - 1]?.prenom : ""}</strong>
</p>
<p>
  Département actuel: <strong>{data && data.length > 0 ? data[data.length - 1]?.libelle_new : ""}</strong>
</p>
        ...
        <table className="table content_2 ">
          <thead>
            <tr>
              ...
              <th scope="col">Ancien département</th>
              <th scope="col">Nouveau département</th>
        ...
        <button type="button" className="btn btn-success btn-lg" data-dismiss="modal" data-toggle="modal" data-target="#myModalAdd">
          <FontAwesomeIcon icon={faPlus} className="add-icon" /> Ajouter un département
        </button><br /><br /><br />
      </div>
